...
export default function SettingsPage() {
  ...
  const [reminderEnd, setReminderEnd] = useState("21:00");
  ...
  useEffect(() => {
    fetch("/api/settings")
      ...
      .then((d) => {
        ...
        setReminderStart(s.reminderStart ?? "09:00");
        setReminderEnd(s.reminderEnd ?? "21:00");
  ...
  async function handleSaveReminder() {
    ...
    await fetch("/api/settings", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ reminderEnabled, reminderStart, reminderEnd }),
    });
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      ...
      <main className="mx-auto max-w-3xl p-6">
        ...
        <div className="mt-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between gap-2">
            <h2 className="text-sm font-medium text-slate-800">跟进提醒</h2>
            <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-600">
              客户跟进日期到期时主动提醒你
            </span>
          </div>
          <p className="mt-1 text-sm text-slate-600">
            每天设定时段内,客户「下次跟进日期」到点或逾期时,bot 会在微信里主动提醒你。关掉开关就完全不再推送。
          </p>
          <div className="mt-4 flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-0.5 text-xs text-slate-500">关掉后机器人不再主动推送跟进提醒</p>
            </div>
            <button
              type="button"
              role="switch"
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            <label className="block">
              <span className="mb-1 block text-sm font-medium text-slate-700">开始时间</span>
              <input
                type="time"
                value={reminderStart}
            ...
            <label className="block">
              <span className="mb-1 block text-sm font-medium text-slate-700">结束时间</span>
              <input
                type="time"
                value={reminderEnd}
          ...
          <span className="mt-1 block text-xs text-slate-500">
            只在开始到结束之间的时段推送;例如 09:00–21:00。机器人每 5 分钟检查一次,不是准点精确。
          </span>
          <div className="mt-4 flex items-center gap-3">
            <button
            ...
            {savedReminder && <span className="text-sm text-emerald-600">已保存 ✓</span>}
          </div>
        </div>
